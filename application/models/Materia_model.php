...
class Materia_model extends CI_Model {
    ...
    public function saveMateria($nombre, $duracionMeses, $regimen, $cantidadParcial,$idMateria) {
         $error="";
        $datos = array();
        $datos['Nombre']=$nombre;
        $datos['DuracionMeses']=$duracionMeses;
        $datos['Regimen']=$regimen;
        $datos['CantidadParcial']=$cantidadParcial;
        $datos['Activo']=1;
        try {
            if($idMateria == 0)
            {
                ...
                $this->db->update('materia', $datos, array('IdMateria' => $idMateria)); 
            }
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
                throw new Exception("Database error:");
                $error= "Error!!! No se pudo Agregar el Materia";
            }
        } catch (Exception $e) {
    ...
    public function listMateriaBy($campo_buscar='', $tipo=NULL) {
        $resultados = array();
        $sql = "SELECT * FROM materia WHERE Activo=1";
        if ($campo_buscar != '' && isset($campo_buscar)) {
            if (is_string($campo_buscar)) {                
                $sql.=" AND ( Nombre LIKE '%" . $campo_buscar . "%')";                
            }
        }
