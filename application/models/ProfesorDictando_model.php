...
class ProfesorDictando_model extends CI_Model {
        ...
        try {
            if($idProfesor == 0)
            {
              if(!$this->existeProfesorLegajo($legajo,$dni))
              {
               ...
               $error="Error!!!!!!!! El Legajo o Dni de Profesor ya Existe " ;  
              }
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                $error= "Error!!! No se pudo Agregar el Profesor";
            }
        } catch (Exception $e) {
    ...
    public function listCursorBy($campo_buscar='', $tipo=NULL) {

        $idUsuario=$this->session->userdata('id_usuario');
        $resultados = array();
        $sql = "SELECT cu.* FROM curso as cu inner join profesor as pe on pe.IdProfesor= cu.IdProfesor
                                   WHERE pe.IdUsuario=".$idUsuario." and cu.Activo=1";
        if ($campo_buscar != '' && isset($campo_buscar)) {
                $sql.=" AND ( cu.Anio LIKE '%" . $campo_buscar . "%')";  
        }              
    ...
    public function listMesaBy($campo_buscar='', $tipo=NULL) {
        ...
        $idUsuario=$this->session->userdata('id_usuario');
        $resultados = array();
        $sql = "SELECT cu.* FROM mesa as cu inner join profesor as pe on pe.IdProfesor= cu.IdProfesor
                                   WHERE pe.IdUsuario=".$idUsuario." and cu.Activo=1";
        if ($campo_buscar != '' && isset($campo_buscar)) {
                $sql.=" AND ( cu.NumeroLibro LIKE '%" . $campo_buscar . "%')";  
        }              
        $objeto = $this->db->query($sql);
        $resultados = $objeto->result();
        return $resultados;
    ...
    public function listAlumnosBy($idCurso) {
        $resultados = array();
        $sql = "SELECT cu.* FROM curso_alumno as cu WHERE cu.IdCurso=".$idCurso." and cu.Activo=1";         
        $objeto = $this->db->query($sql);
        $resultados = $objeto->result();
        return $resultados;
    }
    public function listAlumnosMesaBy($idMesa) {
        $resultados = array();
        $sql = "SELECT cu.* FROM mesa_alumno as cu WHERE cu.IdMesa=".$idMesa." and cu.Activo=1";         
        $objeto = $this->db->query($sql);
        $resultados = $objeto->result();
        return $resultados;
    }
