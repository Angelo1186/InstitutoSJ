<script type="text/javascript" charset="utf-8">
    $(document).ready(function() {   
        //   $('#proximoVencer').modal('show');
        //   $('#inmueblesDesocupado').modal('show');
        //   $('#clientesDeudores').modal('show');
           
      /*     $("#proximoVencer").draggable({
                handle: ".modal-header"
            });
            $("#inmueblesDesocupado").draggable({
                handle: ".modal-header"
            });
            $("#clientesDeudores").draggable({
                handle: ".modal-header"
            });  */
  ...
  function AbriCarrera(idMateria)
  {
/*    $.ajax({
    ...
    $.ajax(
            {   type:'post',
                url:"<?= site_url('Materia/getAjaxMateriaCarrera') ?>" ,
                data:{"idMateria":idMateria}
             }).done(function(data) {
                $('#proximoVencer').modal('show');  
                $("#proximoVencer .modal-body").html(data);
            });
    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header bg-light d-flex">
                <h4 class="ml-auto">Materia</h4>
                <a href="<?= site_url('Materia/agregarMateria') ?>" class="btn btn-primary ml-auto"  title="Agregar Materia"><i class="fas fa-plus"></i> &nbsp; Agregar Materia</a>
            </div>
            <div class="card-body">
                ...
                <fieldset class="border p-2">
                    ...
                    <?= form_open_multipart('Materia', 'id="formbusqueda"') ?>
                    <div class="row">                        
                        <div class="col-md-6">
                        ...
                        <div class="col-md-6">
                            ...
                            <a  href="<?= site_url('Materia/MateriaPDF') ?>"class="btn btn-info" title="Imprimir" target="_blank" ><i class="fa fa-print">GENERAR LISTADO </i></a>
                            </div>
                        </div>
                    </div>
                    <?= form_close() ?>
                    ...
                    <table class="table" id="tabla_clientes">
                        ...
                        <tbody>
                            ...
                            if (count($Materia) > 0) {
                                        ...
                                        <td>
                                            <div class="btn-toolbar mb-3" role="toolbar" aria-label="Toolbar with button groups">
                                                <div class="btn-group mr-2" role="group" aria-label="First group">
                                                    <a  href="<?= site_url('Materia/agregarMateria/' . $Materias->IdMateria) ?>" class="btn btn-primary" title="Editar"><i class="far fa-edit"></i></a>                                                
                                                   <!-- <a  href="<= site_url('clientes/verCliente/' . $Materias->IdMateria . '/cliente') ?>"class="btn btn-secondary" title="Detalles"><i class="fa fa-search-plus"></i></a> -->
                                                    <a  href="<?= site_url('Materia/eliminarMateria/' . $Materias->IdMateria) ?>"class="btn btn-danger" title="Borrar" onclick="return confirm('Esta seguro de Eliminar el Materia???')"><i class="fas fa-trash"></i></a>
                                                    <a  href="#" class="btn btn-secondary" onclick="AbriCarrera(<?=$Materias->IdMateria?>)" title="Agregar Carrera"><i class="fa fa-search-plus"></i></a>     
                                                </div>
                                            </div>
                                        </td>
                                    </tr>
                                ...
                                echo'<tr><td colspan="5" class="alert alert-danger">No se encontraton resultados</td></tr>';
                            ?>
                        </tbody>
                    </table>
                </div>
...
<!-------------------------------------------------------------------------------------------------------------------------------------------------------------------->
<!-- Ç<div class="modal fullscreen-modal fade" id="proximoVencer" tabindex="-1"  role="dialog" aria-labelledby="proximoVencer"> --->
<div class="modal" id="proximoVencer" tabindex="-1"  role="dialog" aria-labelledby="proximoVencer">
  <div class="modal-dialog" role="document" style="margin-top: 15%;">
    <div class="modal-content" >
      <div class="modal-header" style="background-color:#007bff; color:#FFFFFF">
        <h5 class="col-12 modal-title text-center">AGREGAR/MODIFICAR CARRERA</h5>
