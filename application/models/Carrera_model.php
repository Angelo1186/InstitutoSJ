...
class Carrera_model extends CI_Model {
    ...
    public function saveCarrera($nombre, $duracionAnio,$idCarrera) {
         $error="";
        $datos = array();
        $datos['Nombre']=$nombre;
        $datos['DuracionAnio']=$duracionAnio;
        $datos['Activo']=1;
        try {
            if($idCarrera == 0)
            {
                ...
                $this->db->update('carrera', $datos, array('IdCarrera' => $idCarrera)); 
            }
            $this->db->trans_complete();
            if ($this->db->trans_status() === FALSE) {
                throw new Exception("Database error:");
                $error= "Error!!! No se pudo Agregar el Carrera";
            }
        } catch (Exception $e) {
    ...
    public function listCarreraBy($campo_buscar='', $tipo=NULL) {
        $resultados = array();
